...
class FewShotCycleGANModel(BaseModel):
    def name(self):
        return 'FewShotCycleGANModel'
    ...
    def initialize(self, opt):
        ...
        self.netF = networks.define_G(opt.output_nc, opt.input_nc, opt.ngf, opt.which_model_netG, opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids)
        ...
        if self.isTrain:
            ...
            self.netD = networks.define_D(opt.input_nc, opt.ndf, opt.which_model_netD, opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids)
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG.parameters(), self.netF.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def backward_G(self):
        ...
        # perceptual loss
        if self.opt.lambda_content_A > 0:
          in_feat_fake_B = self.instance_norm_layer(self.vgg(fake_B))
          in_feat_real_A = self.instance_norm_layer(self.vgg(self.real_A))

          loss_perc_A = self.opt.lambda_content_A * self.criterionPerc(in_feat_fake_B, in_feat_real_A)
        ...
        # combined loss
        loss_G = loss_G_A + loss_cycle_A + loss_idt_A + loss_idt_B + loss_perc_A
        loss_G.backward()
    ...
    def get_current_errors(self):
        ret_errors = OrderedDict([('D', self.loss_D), ('G', self.loss_G_A), ('Cyc_A', self.loss_cycle_A)])
    ...
    def get_current_visuals(self):
        ...
        if not self.idt_A is None:
            ret_visuals['idt_A'] = util.tensor2im(self.idt_A)
            ret_visuals['idt_B'] = util.tensor2im(self.idt_B)
        if not self.estimated_B is None:
            ret_visuals['enhanced_fake_B'] = util.tensor2im(self.cached_estimated_B)
        return ret_visuals
